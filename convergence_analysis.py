        ...
        def romConv():
            ...
            for i in self.par_space:
                ...
                for m in range(self.number_mc_samples):
                    ...
                    sample_norm_OnlyRom,_ = funcs.computeErrorNorm(solution=np.copy(funcs.u_mean),reference=funcs.true_process)
                    sample_norm,_ = funcs.computeErrorNorm(solution=np.copy(funcs.u_mean_y_std),reference=funcs.true_process)
                    sample_normEasy,_ = funcs.computeErrorNorm(solution=np.copy(funcs.u_mean_y_easy),reference=funcs.true_process)
                    sample_normPred,_ = funcs.computeErrorNorm(solution=np.copy(funcs.u_mean_y_pred_rom),reference=funcs.true_process)
                    ...
                    print(m)
                    print(sample_norm)
                    mc_sig_fom_l.append(funcs.sigd_std)
                    mc_sig_easy_l.append(funcs.sigd_easy)
                    mc_sig_adv_l.append(funcs.sigd_adv)
                self.sigd_fom_list.append(np.median(mc_sig_fom_l))
                self.sigd_easy_list.append(np.median(mc_sig_easy_l))
                self.sigd_adv_list.append(np.median(mc_sig_adv_l))
                ...
                print("MEAN MC") 
                results_list.append(res) 
                filename = "./Results/conv_data_std.dat"
                ...
                filename = "./Results/conv_data_ROM_OnlyRom.dat"
                with open(filename, "a") as file:
                    file.write(str(resOnlyRom)+" at L = "+str(i)+"\n")

            filename = "./Results/sigd_data_fom.dat"
            with open(filename, "a") as file:
                for item in self.sigd_fom_list:
                    file.write(str(item)+" at L = "+str(i)+"\n")
            filename = "./Results/sigd_data_easy.dat"
            with open(filename, "a") as file:
                for item in self.sigd_easy_list:
                    file.write(str(item)+" at L = "+str(i)+"\n")
            filename = "./Results/sigd_data_adv.dat"
            with open(filename, "a") as file:
                for item in self.sigd_adv_list:
                    file.write(str(item)+" at L = "+str(i)+"\n")
